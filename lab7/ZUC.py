...
def Sbox(num, SBOX):
    return SBOX[num >> 4][num & 0xf]


def S(num):
    x0, x1, x2, x3 = num >> 24, (num >> 16) & 0xff, (num >> 8) & 0xff, num & 0xff
    y0, y1, y2, y3 = Sbox(x0, S0), Sbox(x1, S1), Sbox(x2, S0), Sbox(x3, S1)
...
def L1(num):
    return num ^ shift(num, 2) ^ shift(num, 10) ^ shift(num, 18) ^ shift(num, 24)


def L2(num):
    return num ^ shift(num, 8) ^ shift(num, 14) ^ shift(num, 22) ^ shift(num, 30)
...
def LFSRWithInitialisationMode(u):
    global s
    v = ((1 << 15) * s[15] + (1 << 17) * s[13] + (1 << 21) * s[10] + (1 << 20) * s[4] + (257) * s[0]) % 2147483647
    s[16] = (u + v) % 2147483647
    s[16] = 2147483647 if s[16] == 0 else s[16]
    for i in range(16):
        s[i] = s[i + 1]
...
def LFSRWithWorkMode():
    global s
    s[16] = ((1 << 15) * s[15] + (1 << 17) * s[13] + (1 << 21) * s[10] + (1 << 20) * s[4] + (257) * s[0]) % 2147483647
    s[16] = 2147483647 if s[16] == 0 else s[16]
    for i in range(16):
        s[i] = s[i + 1]
...
def install(key, IV):
    ...
    iv = iv[::-1]
    for i in range(16):
        s.append(k[i] << 23 | d[i] << 8 | iv[i])
    s.append(0)


def ZUC(init_key, IV, cnt):
    install(init_key, IV)
    global R1, R2
    R1 = R2 = 0
    for i in range(32):
...
if __name__ == '__main__':
    # ZUC(0, 0, 2)
    # ZUC(0xffffffffffffffffffffffffffffffff, 0xffffffffffffffffffffffffffffffff, 2)
    ZUC(0x338985fedc98cb99c9f9b346bd6cae56, 0x7dd3175c2374c7113bcbbfb339563172, 10)