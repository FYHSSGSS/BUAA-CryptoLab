...
static const char sbox[256] = {0xd6, 0x90, 0xe9, 0xfe, 0xcc, 0xe1, 0x3d, 0xb7, 0x16, 0xb6, 0x14, 0xc2, 0x28, 0xfb, 0x2c,
                               0x05,
...
static const u32 ck[32] = {0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
...
u32 transT_k(u32 p) {
    ...
    r.c[2] = sbox[r.c[2]];
    r.c[3] = sbox[r.c[3]];
    r.u = r.u ^ ((r.u << 13) | (r.u >> 19)) ^ ((r.u << 23) | (r.u >> 9));
    return r.u;
}

u32 *expandKey(u32 *key) {
    u32 k[4] = {key[0] ^ fk[0], key[1] ^ fk[1], key[2] ^ fk[2], key[3] ^ fk[3]};
    int i = 0;
    u32 *ek;
    ek = (u32 *) malloc(sizeof(u32) * 32);
    for (i = 0; i < 32; i++) {
        *(ek + i) = k[0] ^ transT_k(k[1] ^ k[2] ^ k[3] ^ ck[i]);
        k[0] = k[1];
        k[1] = k[2];
        k[2] = k[3];
        k[3] = *(ek + i);
...
u32 *sm4_enc(u32 *p, u32 *ek) {
    u32 *c = NULL;

    u32 v;
    c = (u32 *) malloc(sizeof(u32) * 4);
    *c = p[0];
    *(c + 1) = p[1];
    *(c + 2) = p[2];
    *(c + 3) = p[3];

    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[0]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[1]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[2]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[3]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[4]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[5]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[6]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[7]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[8]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[9]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[10]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[11]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[12]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[13]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[14]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[15]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[16]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[17]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[18]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[19]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[20]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[21]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[22]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[23]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[24]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[25]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[26]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[27]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[28]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[29]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[30]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
    v = *c ^ transT(*(c + 1) ^ *(c + 2) ^ *(c + 3) ^ ek[31]);
    *c = *(c + 1);
    *(c + 1) = *(c + 2);
    *(c + 2) = *(c + 3);
    *(c + 3) = v;
...
u32 *sm4_dec(u32 *c, u32 *key) {
    ...
    *(p + 3) = c[3];
    ...
    for (i = 0; i < 32; i++) {
        v = *p ^ transT(*(p + 1) ^ *(p + 2) ^ *(p + 3) ^ ek[31 - i]);
        *p = *(p + 1);
        *(p + 1) = *(p + 2);
        *(p + 2) = *(p + 3);
        *(p + 3) = v;
