...
def DEScrypt(secret, message, tp):
    ...
    if tp != 1:
        rk = rk[::-1]
    print(list(map(hex, rk)))
    first = replacement(message, IP, 64)
    left = first >> 32
    right = last(first, 32)
    for i in range(16):
        print(i, ':')
        print(hex((left << 32) | right))
        print(hex(replacement(right, replace3, 32)))
        r_int = replacement(right, replace3, 32) ^ rk[i]
        print(hex(r_int))
        print(hex(confuse(r_int)))
        temp = replacement(confuse(r_int), replace4, 32)
        print(hex(temp))
...
if __name__ == "__main__":
    ...
    print(hex(a1))
    # print(hex(DEScrypt(0x0f1571c947d9e859, 0xda02ce3a89ecac3b, 0)))
    end_time = time.time()
