...
flag = b'flag{s0_des_is_really_not_secure_right?}'
MENU = br'''[+] 1.function
[+] 2.check_answer
[+] 3.exit
'''
...
class Task(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        self.send(b'[++++++++++++++++] FLAG [++++++++++++++++]')
        self.send(cipher)
        self.send(b'[+] You have up to 65537 chances to obtain the ct corresponding to the pt your input(<= 64 bits and hex required).')
        for i in range(65537):
            self.send(b"[+] Plz input: ")
            pt = self.recv().strip()
            try:
                pt = unhexlify(pt)
                assert (len(pt) <= 8)
                ct = enc(pt)
                self.send(hexlify(ct))
            except:
                self.send(b'[*] Read the above carefully!')
                break
        self.request.close()
